      ...
      <Card style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: 'grid', gap: 10 }}>
          {Object.entries(THEMES).map(([key, t]) => (
            ...
            >
              <div style={{ width: 32, height: 32, borderRadius: 99, background: `linear-gradient(135deg, ${t.primary}, ${t.accent})` }} />
              <div>
                <div style={{ fontWeight: 700, color: '#0F172A', fontSize: 14 }}>{t.name}</div>
                <div style={{ fontSize: 11, color: '#94A3B8' }}>{t.primary}</div>
              </div>
              {themeKey === key && <div style={{ marginLeft: 'auto', color: t.primary, fontWeight: 800, fontSize: 16 }}>✓</div>}
            </button>
          ))}
        </div>
      </Card>
